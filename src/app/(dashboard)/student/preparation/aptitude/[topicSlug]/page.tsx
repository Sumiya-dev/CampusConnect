...
import { APTITUDE_TOPICS, findTopicBySlug, matchesTopic } from '@/lib/preparation/topics';
...
export default async function AptitudeTopicMaterialsPage({ params }: PageProps) {
  ...
  const topic = findTopicBySlug(APTITUDE_TOPICS, topicSlug);

  if (!topic) {
    notFound();
  }

  const allMaterials = await getPreparationMaterials({ category: 'Aptitude' }, user.id);
  ...
  return (
    <PageContainer
      ...
      breadcrumbs={[
        ...
        { label: 'Aptitude & Reasoning', href: '/student/preparation/aptitude' },
        { label: topic.title },
      ]}
      actions={
        <Link href="/student/preparation/aptitude">
          <Button
            variant="outline"
            size="sm"
